...
One field is legal on **every** kind rather than being kind-specific: the optional `narration` string, the spoken line for the user. It is presentation only - it carries no routing meaning, any kind may omit it, and dropping it changes nothing about what the framework does. The engine authors it because the engine already knows deterministically which stage this is, what scope resolved, and what comes next; the conductor relays that text instead of inferring a description from the mechanism it can see. Being legal everywhere means a new emission point can carry a line without a schema change. Values stay to one sentence, two at most: a `load-steering` directive's rule payload is chunked against the transport budget with limited headroom, so `narration` is never what pushes a directive over it.
...
These six `SWARM_*` events are part of the 74-event audit taxonomy (see [State Machine](12-state-machine.md)). On an exit-2 envelope the conductor takes the baton back - failure always halts and re-engages the human regardless of autonomy mode.
...
- **The transitions `report` commits** - the workflow / phase / stage machines and the 74-event audit taxonomy. See [State Machine](12-state-machine.md).
