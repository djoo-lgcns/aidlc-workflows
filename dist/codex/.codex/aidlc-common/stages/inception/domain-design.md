...
Load aidlc-aws-platform-agent persona from `agents/aidlc-aws-platform-agent.md` and knowledge from `.codex/knowledge/aidlc-aws-platform-agent/` for dependency-service awareness (a component may depend on a managed service — that dependency is captured, but the service itself is not a component).
...
`bun .codex/tools/aidlc-orchestrate.ts report --stage domain-design --result <outcome>`.
...
`bun .codex/tools/aidlc-utility.ts recompose --add units-generation`
