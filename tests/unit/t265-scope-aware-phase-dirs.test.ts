...
//
// t265: SCOPE-AWARE PHASE DIRS at intent creation (deterministic, no-LLM,
...
function mkTemp(tag: string): string {
  const d = mkdtempSync(join(tmpdir(), `aidlc-t265-${tag}-`));
  tempDirs.push(d);
  return d;
}
...
describe("t265 scope-aware phase dirs at intent creation", () => {
  // === (a) NARROW SCOPE ====================================================
