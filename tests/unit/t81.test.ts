...
describe("t81 aidlc-state practices-event — bolt-plan-marker-conflict override (migrated from t81-bolt-plan-override.sh, plan 4)", () => {
  ...
  test("3: framework event count pinned at 78 (no bump from this PR's discriminator reuse)", () => {
    ...
    // new event. The framework total is 74: the v0.6.0 Wave 4 milestone 16
    ...
    // REVIEW_REQUESTED + REVIEW_COMPLETED (reviewer-enforcement RFC Track 1, +2),
    // plus UNIT_STARTED + UNIT_PAUSED + UNIT_RESUMED + UNIT_COMPLETED (the
    // unit lifecycle receipts, +4 → 78).
